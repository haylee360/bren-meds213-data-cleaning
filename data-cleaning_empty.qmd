...
```{r}
# str_detect(snowsurvey_csv$Snow_cover, "[0123456789]", negate = TRUE)

# snowsurvey_csv %>%
#   count(Snow_cover) %>% 
#   filter(Snow_cover == Snow_cover[str_detect(snowsurvey_csv$Snow_cover, "[0123456789]", negate = TRUE)])
...
```{r}
# snowsurvey_fixed <- replace_with_na(snowsurvey_csv, 
#                                     replace = list(Snow_cover = ".",
#                                                    Land_cover = ".",
#                                                    Water_cover = ".", 
#                                                    Total_cover = ".")) 
...
species_long <- species_csv %>% 
  pivot_longer(
    ...
    # explicitly make the type character because we have some X's and other weird things
